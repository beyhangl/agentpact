"""Tool-argument predicates — validate and constrain the *arguments* of a call.

Where ``tools.py`` governs *which* tools may run, these govern *what* they may
be called with: JSON-Schema shape, destructive-command denylist, filesystem
sandbox, per-field value allow/deny, and required disclosure phrases.
"""

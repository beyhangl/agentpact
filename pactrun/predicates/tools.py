"""Tool-name policy predicates — control *which* tools an agent may/must call.

Argument-level guards live in ``tool_args`` / ``network`` / ``consent``;
cross-run exfiltration guards live in ``exfil``.
"""
